...
// Simple helper functions (inline to avoid dependency issues)
...
const DocumentSummarizerPageFixed = () => {
  const { user } = useAuth();
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setUploadForm({ title: '', subject: '', notes: '', file: null });
        setUploadProgress(0);
        fetchDocuments();
        fetchDashboard();
        setError('');
            ...
            {selectedDocument && (
              <Box>
                <Box display="flex" gap={1} mb={3}>
                  ...
                  <Chip label={formatFileSize(selectedDocument.file_size)} size="small" variant="outlined" />
...
export default DocumentSummarizerPageFixed;
